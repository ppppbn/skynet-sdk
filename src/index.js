(function(window) {
	...
	/* Generate guid */
	SkynetUtils.generateGuid = function() {
		return 'aaaaaaaa-aaaa-4aaa-baaa-aaaaaaaaaaaa'.replace(/[ab]/g, function(ch) {
			let digit = Math.random()*16|0, newch = ch == 'a' ? digit : (digit&0x3|0x8);
  ...
  Skynet.metadata = {
	...
	Skynet.uniqueGuid = true;
	/* Generate guid for page session */
	Skynet.guid = SkynetUtils.generateGuid();
	...
	Skynet.sendError = function(index, url, fileLocation, lineNumber, columnNumber, uniqueId, info, msg) {
		try {
			Skynet.sendErrorInternal(index, url, fileLocation, lineNumber, columnNumber, uniqueId, info, msg)
		} catch (e) {
	...
	Skynet.sendErrorInternal = function(index, url, fileLocation, lineNumber, columnNumber, uniqueId, info, msg) {
    ...
    if (Skynet.additionalInfo) {
      errorMessage += '\n Information:' + Skynet.additionalInfo;
        ...
        Skynet.qsIgnore = Skynet.qsIgnore.map(item => item.toLowerCase());
			...
			let i = Skynet.errorIndex;
			let uniqueId = Skynet.uniqueGuid ? SkynetUtils.generateGuid() : Skynet.guid;
			let info = Skynet.additionalInfo;
			Skynet.sendError(i, url, fileLocation, lineNumber, columnNumber, uniqueId, info, msg);
		}
		return true;
	}
