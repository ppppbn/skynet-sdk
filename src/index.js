...
(function (window) {
  ...
  SkynetUtils.matchDomains = function (loc, domains) {
    return new RegExp(
      '(' +
        domains.join('|').replace(/\./g, '\\.').replace(/\*/g, '\\w+') +
        ')',
    ).test(loc);
  };
  ...
  Skynet.messageWhitelist = [
    'Uncaught TypeError: C is not a function',
    `TypeError: C is not a function. (In 'C(!0)', 'C' is undefined)`,
  ];
  /* Skynet project ID */
  Skynet.projectId = '';
  ...
  window.onerror = function (msg, fileLocation, lineNumber, columnNumber) {
    ...
    if (typeof Skynet.fnPreviousOnErrorHandler === 'function') {
      ...
      Skynet.fnPreviousOnErrorHandler(
        msg,
        fileLocation,
        lineNumber,
        columnNumber,
  ...
  Skynet.sendErrorInternal = function (
    index,
    url,
    fileLocation,
    lineNumber,
  ...
  Skynet.errorTrap = function (msg, fileLocation, lineNumber, columnNumber) {
    ...
    if (
      Skynet.domainWhitelist.length > 0 &&
      !SkynetUtils.matchDomains(fileLocation, Skynet.domainWhitelist)
    ) {
      console.log(
        `Skynet - report ignored because ${fileLocation} did not match the whitelist`,
      );
      return;
    }

    if (
      Skynet.domainIgnore.length > 0 &&
      SkynetUtils.matchDomains(fileLocation, Skynet.domainIgnore)
    ) {
      console.log(
        `Skynet - report ignored because ${fileLocation} matched the blacklist`,
      );
      return;
    }

    if (
      Skynet.ignoreCrossOriginErrors &&
      (msg === 'Script Error' || msg === 'Script error.') &&
      Number(lineNumber) === 0
    ) {
      console.log(
        'Skynet - cross origin script error ignored because no additional error info supplied.',
      );
      return;
    }

    if (
      Skynet.messageWhitelist.length > 0 &&
      Skynet.messageWhitelist.includes(msg)
    ) {
      console.log(
        'Skynet - error ignored because the message is in whitelist.',
      );
      return;
    }
    ...
    if (Skynet.debugMode) {
      ...
      if (Skynet.qsIgnore.length > 0) {
        ...
        if (fileLocation && fileLocation.length > 1) {
          fileLocation = SkynetUtils.stripQueries(
            SkynetUtils.parseURL(fileLocation),
          );
        }
      }
  ...
  Skynet.sendProfilingResourceLogs = function () {
    try {
      ...
      const performanceResourceTimingLogs =
        window?.performance?.getEntriesByType('resource') || [];
      const performanceNavigationTimingLogs =
        window?.performance?.getEntriesByType('navigation') || [];

      if (
        !performanceResourceTimingLogs?.length &&
        !performanceNavigationTimingLogs?.length
      ) {
        return;
      }
  ...
  function logAnalytics(metric) {
    Skynet.sendPerformanceDataInternal(metric.name, {
      ...
      element:
        metric?.entries?.[0]?.url ||
        (metric?.entries?.[0]?.element
          ? stringify(metric?.entries?.[0]?.element)?.slice(0, 300)
          : null),
    });
  }
  ...
  function observePerformMarkAndMeasureLogs(snapshotRequestId) {
    const observer = new PerformanceObserver(list => {
      const whitelistEntries = list
        .getEntries()
        .filter(entry => entry.name.includes('Telio'));
  ...
  function saveEvents(snapshotRequestId, sequenceNumber, events) {
    try {
      const backendUrl = Skynet.backendUrl || Skynet.defaultBackendUrl;
      const url = `${backendUrl}/session-replay`;
